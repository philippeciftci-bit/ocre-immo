...
- [x] ~~**URGENCE — SMTP signup fiable PHPMailer (mails non reçus + alerting silencieux)**~~ — **livrée M/2026/05/08/29** (root cause : `ocre_send_email()` utilisait `/usr/sbin/sendmail` qui acceptait le mail (exit 0 = OK) mais Postfix ne délivrait pas réellement → ✅ ✅ pastilles UI mensonge + alerting `_alert_email_failure` jamais déclenché car retour `true`. Fix : refonte `lib/email_sender.php` avec nouvelle `send_mail()` qui tente service externe HTTP API en priorité (clé déjà configurée dans `_smtp_config.php` hors-git), fallback sendmail si service externe KO. Retourne `{ok, error, message_id, provider}`. Shim `ocre_send_email()` retourne bool pour rétro-compat zero-touche du code appelant. Smoke test E2E réel philippe.ciftci@gmail.com : provider=externe, message_id=8db89a82-... Logs `/var/log/ocre/email-sender.log` distinguent `SENT_EXTERN` vs `SENT_SENDMAIL` vs `FAIL_*`. Sender actuel : `<service externe (legacy)>` (domain ocre.immo non vérifié cote service externe — TODO M+1 vérification DNS).)
- [x] ~~**Smoke tests Ocre tenant éphémère (élimine dépendance zefk)**~~ — **livrée M/2026/05/08/36** (Plus de dépendance au tenant fixe `zefk` qui disparaît à chaque Reset TOTAL. Nouveau `tests/smoke_tenant.php` CLI 3 actions setup/teardown/cleanup_orphans. `tests/run_all.sh` refait avec trap teardown EXIT (cleanup garanti même si crash) + cleanup_orphans au démarrage. Setup crée user `smoke+ts@ocre-internal.test` + provisionne `ocre_wsp_smoke-ts` via `_provision.php` existant + crée session token. ENV vars exportées (SMOKE_TENANT_SLUG, SMOKE_ADMIN_TOKEN). `helpers.sh` : TENANT_BASE=app.ocre.immo + header `X-Tenant-Slug` dynamique au lieu de zefk.ocre.immo en dur. Test E2E standalone OK : setup crée DB+user+token, teardown clean tout (smoke_users post=0).)
- [x] ~~**URGENCE — Signup HTTP 500 "Erreur serveur" sur étape 4 wizard (require_once smtp_config fatal)**~~ — **livrée M/2026/05/08/39** (Philippe inscription test 12:00 ozkan44000@gmail.com → "⚠️ Erreur serveur" au click "Créer mon compte". ROOT CAUSE diagnostiqué via curl repro : `Failed opening required '/opt/ocre-app/api/lib/../_smtp_config.php'`. Le fichier `_smtp_config.php` avait perms `600 root:www-data` (créé en M31) mais PHP-FPM tourne en `user=www-ocre group=www-data` → owner `root` ≠ `www-ocre` ET perms `---` pour group → fatal `require` qui plantait `agents_register.php` AVANT l'INSERT user. (1) Fix immédiat : `chmod 640 /opt/ocre-app/api/_smtp_config.php` (group www-data peut lire). Retest curl post-chmod : HTTP 201 + email_sent=true + user_id=60 créé OVH SMTP. (2) Fix durable code : `email_sender.php` `is_readable()` au lieu de `file_exists()` + `try/catch Throwable` autour du `require` → si config unreadable retourne `ok=false` au lieu de fatal. (3) Architecture résilience : `agents_register.php` wrappe `_send_activation_email()` dans `try/catch Throwable` (pending branch + INSERT branch) → SI SMTP plante, le user EST créé en DB et l'alerting `_alert_email_failure` se déclenche normalement, plus jamais de blocage HTTP 500.)
- [x] ~~**Investigation 25 fails smoke tests post-M36 (catégorisation + fixes ciblés)**~~ — **livrée M/2026/05/08/38** (Diagnostic catégorisé : (A) 6 fails admin/* → user smoke avec role='agent' n'avait pas accès → fix `tests/smoke_tenant.php` setup utilise role='super_admin'. (B) 12 fails endpoints tenant (clients/matches/events/notifications/edit_consent/preferences/calendar/exports/recycle_bin/feature_flags) → ROOT CAUSE : nginx vhost `ocre-app.conf` OVERRIDE le header client `X-Tenant-Slug` avec `fastcgi_param HTTP_X_TENANT_SLUG $slug` extrait du subdomain (et fallback "ozkan" sur app.ocre.immo). Mes calls `app.ocre.immo + header` étaient routés vers DB `ocre_wsp_ozkan` inexistante → 503. Fix : utiliser `https://<smoke-slug>.ocre.immo` (DNS wildcard *.ocre.immo en place) → nginx extrait slug du subdomain natif → route correctement vers `ocre_wsp_smoke-<ts>` provisionnée. (C) 0 vrai bug code applicatif. **Résultat re-run smoke local : 33 pass · 0 fail · 1s**.)
- [x] ~~**Cards Vue d'ensemble cliquables → raccourcis détails (Sessions/Users cross/hash routing)**~~ — **livrée M/2026/05/08/37** (Cards Vue d'ensemble super-admin transformées en raccourcis cliquables vers détails. (1) CSS `.sa-card.clickable` : cursor pointer + hover transform translateY(-2px) + border-top 4px + shadow renforcée + flèche `→` indicateur muted opacity 0.4 → 1 au hover. `.is-empty` opacity 0.7 si compteur=0. Accessibilité : tabindex, role=button, aria-label, focus-visible outline accent. (2) Hash routing JS : `parseHashTab()` + `goToTab(tab)` + listener `hashchange` synchronise URL `#tab` avec onglet actif sans reload. Permet bookmark direct. (3) Mapping cards → cibles : Workspaces/WSP→workspaces, WSC→clients, Users→users, Sessions→sessions, Pending→pending, Actions admin→audit. (4) 2 nouveaux onglets dans nav : "Sessions" + "Users" (TAB_ORDER 7 entries). (5) Endpoint `api/superadmin_sessions.php` 3 actions list/force_logout/logout_all_except_me + audit Telegram. Garde-fou : pas de logout self. (6) Endpoint `api/superadmin_users_xt.php` list_all (filtres q/role/status) + delete (garde-fou super_admin self + confirm explicite). (7) `renderSessionsTab()` table densité M32 avec colonnes Email/Rôle/Workspace/IP/UA/Créée/Expire/Actions, badge "moi" pour session courante, bouton 🚪 force_logout par session + bouton "Déconnecter tous" en haut. (8) `renderUsersTab()` table densité M32 avec bordure gauche colorée par status (green/orange/red), 10 colonnes Email/Nom/Rôle/Statut/Workspace/Tel(E.164)/LastLogin/Inscrit/🗑️.)
