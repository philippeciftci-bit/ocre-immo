...
if ($action === 'generate') {
    ...
    $stmt = db()->prepare("INSERT INTO magic_tokens (user_id, token_hash, expires_at) VALUES (?, ?, DATE_ADD(NOW(), INTERVAL ? DAY))");
    $stmt->execute([$user_id, $hash, MAGIC_TTL_DAYS]);
    $url = 'https://ocre.immo/app/api/auth_magic.php?action=consume&mt=' . urlencode($token);
    echo json_encode(['ok'=>true, 'url'=>$url, 'user_email'=>$u['email'], 'user_id'=>$user_id, 'expires_in_days'=>MAGIC_TTL_DAYS]);
    exit;
}
...
if ($action === 'consume') {
    ...
    $hash = hash('sha256', $token);
    $stmt = db()->prepare("SELECT id, user_id, expires_at, consumed_at FROM magic_tokens WHERE token_hash = ? LIMIT 1");
    $stmt->execute([$hash]);
    $row = $stmt->fetch();
    if (!$row) { http_response_code(404); magicErrorPage('Lien introuvable.'); exit; }
    if ($row['consumed_at']) { http_response_code(410); magicErrorPage('Lien déjà utilisé. Demande un nouveau lien.'); exit; }
    if (strtotime($row['expires_at']) < time()) { http_response_code(410); magicErrorPage('Lien expiré (validité 7 jours). Demande un nouveau lien.'); exit; }
