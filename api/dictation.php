...
Exemple : "J'ai rencontré Marc, budget 250kEUR, achète un Riad à rénover Médina, organiser un RDV demain"
...
- 'euros'/'EUR' → devise=EUR ; 'dirhams'/'MAD'/'DH' → devise=MAD
...
function heuristicExtract($transcript) {
    ...
    } elseif (preg_match('/(\d[\d\s\.]{3,}\d)\s*(euros?|EUR|dirhams?|MAD|DH|DHs)/iu', $t, $mm)) {
    ...
    // Devise
    if (preg_match('/\b(euros?|EUR|EUR)\b/iu', $t)) $out['devise'] = 'EUR';
