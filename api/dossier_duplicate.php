<?php
// M/2026/05/05/8 — M-Swipe-Dupliquer-Confreres : duplique un dossier (brouillon).
// Backend : INSERT clients (copie source) sauf id+timestamps+share/publication/workspace.
...
try {
  ...
  $uid = (int)$row['user_id'];
  // Resoudre tenant : recuperer wsp_slug du source via meta
  $w = $pdo->prepare("SELECT wsp_slug FROM clients_meta WHERE client_id=? LIMIT 1");
  try { $w->execute([$src]); $wsp = $w->fetchColumn(); } catch (Throwable $e) { $wsp = null; }
  // Fallback : utiliser la connexion db() (single tenant) si pas de meta
  ...
  // INSERT dynamique a partir des cles
  ...
  echo json_encode(['ok'=>true, 'new_dossier_id'=>$newId]);
} catch (Throwable $e) {
