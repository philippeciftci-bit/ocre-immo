...
if ($method === 'GET') {
    ...
    if ($action === 'list') {
        // Joins users pour email/role
        $sql = "SELECT s.id, s.token, s.user_id, s.expires_at, s.ip, s.user_agent,
...
if ($action === 'force_logout') {
    ...
    if ($sid <= 0) jout(['ok' => false, 'error' => 'session_id required'], 400);
    // Garde-fou : ne pas déconnecter sa propre session courante.
    $st = $meta->prepare("SELECT user_id FROM sessions WHERE id = ?");
    $st->execute([$sid]);
    $row = $st->fetch();
    ...
    $del = $meta->prepare("DELETE FROM sessions WHERE id = ?");
    $del->execute([$sid]);
    @file_put_contents($LOG, "[" . date('c') . "] sa#" . $user['id'] . " force_logout session_id=$sid user_id=" . $row['user_id'] . "\n", FILE_APPEND);
    @shell_exec('/root/bin/notify --project ocre --priority warning --phase warn --mission-id ' . escapeshellarg('SUPERADMIN-SESSIONS/' . time()) . ' --title ' . escapeshellarg('[OCRE] Force logout session') . ' --body ' . escapeshellarg("session_id=$sid user_id=" . $row['user_id'] . " by sa=" . $user['email']) . ' >/dev/null 2>&1 &');
