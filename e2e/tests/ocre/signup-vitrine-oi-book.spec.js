// M_PLAYWRIGHT_OCRE_PARCOURS — signup nouveau user via vitrine ocre.immo → Oi Book
const { test, expect } = require('@playwright/test');
const { getMagicLinkFromDb, cleanupTestUser, genTestEmail, collectConsoleErrors } = require('./helpers/common');

test.describe('Signup vitrine ocre.immo → Oi Book', () => {
  let email;
  test.beforeEach(() => { email = genTestEmail('-book'); });
  ...
  test('Parcours complet : home → tuile → popup → form → magic link → app', async ({ page }) => {
    ...
    await page.click('a[href*="oi-book"]');
    await expect(page).toHaveURL(/oi-book/);
    ...
    await expect(page).toHaveURL(/oi-book|app\.ocre\.immo/, { timeout: 10000 });
