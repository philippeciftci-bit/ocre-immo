// M_PLAYWRIGHT_OCRE_PARCOURS — signup nouveau user via vitrine ocre.immo → Oi Estimer
const { test, expect } = require('@playwright/test');
const { getMagicLinkFromDb, cleanupTestUser, genTestEmail, collectConsoleErrors } = require('./helpers/common');

test.describe('Signup vitrine ocre.immo → Oi Estimer', () => {
  let email;
  test.beforeEach(() => { email = genTestEmail('-estimer'); });
  ...
  test('Parcours complet : home → tuile → popup → form → magic link → app', async ({ page }) => {
    ...
    await page.click('a[href*="oi-estimer"]');
    await expect(page).toHaveURL(/oi-estimer/);
    ...
    await expect(page).toHaveURL(/oi-estimer|app\.ocre\.immo/, { timeout: 10000 });
