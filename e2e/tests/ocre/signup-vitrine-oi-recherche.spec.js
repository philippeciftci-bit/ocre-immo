// M_PLAYWRIGHT_OCRE_PARCOURS — signup nouveau user via vitrine ocre.immo → Oi Recherche
const { test, expect } = require('@playwright/test');
const { getMagicLinkFromDb, cleanupTestUser, genTestEmail, collectConsoleErrors } = require('./helpers/common');

test.describe('Signup vitrine ocre.immo → Oi Recherche', () => {
  let email;
  test.beforeEach(() => { email = genTestEmail('-recherche'); });
  ...
  test('Parcours complet : home → tuile → popup → form → magic link → app', async ({ page }) => {
    ...
    await page.click('a[href*="oi-recherche"]');
    await expect(page).toHaveURL(/oi-recherche/);
    ...
    await expect(page).toHaveURL(/oi-recherche|app\.ocre\.immo/, { timeout: 10000 });
