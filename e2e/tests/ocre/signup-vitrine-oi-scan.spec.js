// M_PLAYWRIGHT_OCRE_PARCOURS — signup nouveau user via vitrine ocre.immo → Oi Scan
const { test, expect } = require('@playwright/test');
const { getMagicLinkFromDb, cleanupTestUser, genTestEmail, collectConsoleErrors } = require('./helpers/common');

test.describe('Signup vitrine ocre.immo → Oi Scan', () => {
  let email;
  test.beforeEach(() => { email = genTestEmail('-scan'); });
  ...
  test('Parcours complet : home → tuile → popup → form → magic link → app', async ({ page }) => {
    ...
    await page.click('a[href*="oi-scan"]');
    await expect(page).toHaveURL(/oi-scan/);
    ...
    await expect(page).toHaveURL(/oi-scan|app\.ocre\.immo/, { timeout: 10000 });
